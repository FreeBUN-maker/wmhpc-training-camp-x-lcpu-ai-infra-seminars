...
答：严格串行的算法由于数据依赖无法利用GPU的并行计算能力，真正使用的GPU算力远远不足
\prob{1.3}{CONCEPT}
...
warp & GPU调度thread的最小单元 & SM中的一个分区 & L0, L1 (不含shared memory) & warp之间的通讯通过L0, L1进行。GPU调度时天然等待一个warp中的所有 \\
block / CTA & thread协作的基本单位 & SM & L0, L1 (含shared memory) & 使用cuda同步机制，通信通过shared memory进行 \\
...
答：(a) 为了保证CPU读数据的时候所有thread都已经执行完，CPU能得到正确的结果。原先的版本中这个同步发生在CUDA_CHECK_KERNEL()这个函数里。
(b) Unified memory版本更快。用的是5090的卡。这个更快的原因可能是用Unified memory模式的时候，cuda内部会将host到device的数据搬运和gpu的计算任务进行overlap，从而减少了整体的延时。
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：错误。只要两个kernel之间没有执行同步操作，他们内部的线程执行就可能是乱序的。
...
答：出bug的原因在于给每个block开了过高的线程数。实验环境的GPU是5090，最大线程数/SM为1536，而原程序开了2048.
之所以原来不加CHECK_CUDA_KERNEL()不会有任何反映，是因为程序中没有检查cuda kernel的错误，也没有执行synchronize，直接读了memory里面的结果，因此报错
只来源于评测程序中发现结果不对。
...
答：这种写法的价值在于可以将一个Kernel需要使用的算力与其问题规模解耦，这样就可以自由地为不同的kernel分配算力。这样做在性能上付出的代价是用的lane减少了，
即GPU的算力没有用满
...
答：(a) 顺序由每个thread完成的先后顺序决定; (b) 程序的正确性不能依赖block的执行顺序。这条限制是scalable programming model对于程序提出要求。
...
答：标准做法是通过main memory，即GPU的显存来传递数据，并调用\texttt{cudaDeviceSynchronize}来确保同步性。
...
constant & 整个grid & 应用程序 & 偏外 & 编译器 \\
L1 / L2 cache & 单个线程 & 线程 & 片上 & 编译器 \\
\bottomrule
\end{tabular}
\end{center}
...
答：性能差别测不出来是因为constant类型的数组也是存在globa memory里面的，读取速率没有发生变化。constant cache的真正优势在于多个kernel都需要访问这个
常数数组的时候可以避免重复搬运常数，让它常驻显存。
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：错误。unified memory允许CPU和GPU同时访问某一块内存空间，只要没有因为程序中的同步等操作导致lock。
\end{enumerate}
