...
\setCJKmainfont[
  ...
  SlantedFont={NotoSerifSC-Regular}
]{NotoSerifSC-Regular}
\setCJKsansfont[
  Path={C:/WINDOWS/fonts/},
  Extension={.ttf},
  ...
  SlantedFont={NotoSansSC-Regular}
]{NotoSansSC-Regular}
\setCJKmonofont[
  Path={C:/WINDOWS/fonts/},
  Extension={.ttf},
...
% 答案框
\newtcolorbox{answerbox}{enhanced, breakable,
  colback=white, colframe=accent!70, boxrule=0.6pt, arc=1.2pt,
  left=6pt, right=6pt, top=4pt, bottom=4pt,
  before upper={\setlength{\parskip}{0.35em}}}
\NewDocumentCommand{\answerline}{+m}{\begin{answerbox}\textbf{答：}#1\end{answerbox}}
...
项目 & 你的卡 \\
\midrule
型号 / compute capability & NVIDIA GeForce RTX 5090 / \\
SM 数量 & 170\\
warp 大小 & 32 \\
...
\begin{enumerate}[label=(\alph*)]
  ...
  \answerline{错误。GPU的算力是所有SIMT处理核算力的加总，单核性能不一定比得上CPU的单核。}
  \item HBM 的“高带宽”指大块连续访问时的吞吐，零散的随机访问达不到标称值。
  \answerline{正确。零散的访问地址可能导致映射到HBM中的物理地址非常集中，无法利用HBM内部的并行度，用不满带宽。}
  \item 严格串行的迭代算法（每步依赖上一步的结果），即使换一块算力更强的 GPU 也快不了多少。
  \answerline{正确。严格的串行算法无法通过SIMT的方式来进行访存和计算的overlap，因此本质上是用单个core进行串行计算，而GPU的算力提升一般来源于核心数量的提升，单核性能提升不显著。}
  \item “算力 1000 TFLOPS”意味着每次运算的延迟是 $10^{-15}$ 秒。
  \answerline{错误。算力是一个吞吐上的概念，1000TFLOPS也可能是用 $10^{-13}$ 秒完成100OPS的计算。}
\end{enumerate}

\prob{1.2}{CONCEPT}
Session 1 讲座里提过“N 方过百万”这个例子。总计算量 $10^{12}$ FLOP 在当代 GPU 上的运算时间大概是毫秒级，那为什么一个严格在线的串行算法仍然做不到几秒内跑完？（从“延迟”和“吞吐”的角度考虑）
\answerline{严格串行的算法由于数据依赖无法并行执行，此时吞吐不能靠提高计算的并行度来提高，而是严格与延迟成反比，因此在GPU这种单核能力弱、靠提高并发度来
提高算力的平台上，无法达到很高的处理速度。}
...
warp & GPU调度thread的最小单元 & SM中的一个分区 & L0, L1 (不含shared memory) & warp之间的通讯通过L0, L1进行，通过CUDA提供的\texttt{warp\_primitive}调用 \\
block / CTA & thread协作的基本单位 & SM & L0, L1 (含shared memory) & 使用\texttt{\_\_syncthreads}同步机制，通信通过shared memory进行 \\
grid & 一次函数调用中启用的顶层数据结构，由若干block组成 & GPU & 全局 & 通过main memory传递数据 \\
\bottomrule
\end{tabular}
\end{center}
...
\answerline{SIMD与SIMT的区别在于thread和pc的数量，SIMD中是一个thread, 一个pc同时对多个data进行同一种操作，而SIMT则是同时调度多个thread的执行，每个thread有自己的pc并且可以单独执行。
判断正误：错误。GPU中同一个warp内thread的路径不同时会退化为串行执行，依次执行每个branch并mask掉不经过这个branch的thread，导致部分thread挂起，对应的硬件空转，降低硬件利用率}
...
\midrule
CPU 单线程 & 10.234 & 2.44 \\
GPU \texttt{<<<1,1>>>} & 137.703 & 32.83 \\
GPU \texttt{<<<1,256>>>} & 2.377 & 0.57 \\
GPU 铺满 grid & 0.023 & 0.01 \\
\bottomrule
\end{tabular}
\end{center}

\answerline{(a) GPU的单核能力弱于CPU，因为CPU中有分支预测以及prefetch等手段来给程序喂数据，降低流水线stall的概率，而GPU中没有这种技术，导致单核计算性能明显更弱；
(b) GPU加速计算靠的是多个处理单元并行计算来摊薄总的访存延时}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \answerline{\texttt{\_\_global\_\_}}
  \item 只会被 kernel 调用的辅助函数。
  \answerline{\texttt{\_\_device\_\_}}
  \item host 和 device 代码都要调用的小工具函数。
  \answerline{\texttt{\_\_host\_\_ \_\_device\_\_}}
  \item 整个 kernel 运行期间不变、所有线程都要读的系数表。
  \answerline{\texttt{\_\_constant\_\_}}
  \item block 内线程共享的暂存数组。
  \answerline{\texttt{\_\_shared\_\_}}
\end{enumerate}
...
PASS
rm bin/m2_first_kernel/02_vector_add_um
\end{lstlisting}

\begin{lstlisting}
Unified Memory版本：
./bin/m2_first_kernel/02_vector_add_um
搬运 + kernel + 读回: 91.7 ms
PASS
rm bin/m2_first_kernel/02_vector_add_um
\end{lstlisting}
...
\answerline{(a) 为了保证CPU读数据的时候所有thread都已经执行完，CPU能得到正确的结果。原先的版本中这个同步发生在\texttt{CUDA\_CHECK\_KERNEL()}这个函数里调用cudaDeviceSynchronize()的这个函数。
(b) Unified memory版本更快。用的是5090的卡。这个更快的原因可能是用Unified memory模式的时候，GPU需要的数据按页迁移，而不是像原来一样调用一个同步的memcpy函数，
这让cuda内部有将host到device的数据搬运和gpu的计算任务进行overlap的机会，从而减少了整体的延时。}

\prob{2.4}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item \texttt{vectorAdd<<<...>>>(...)} 这条语句返回时，kernel 一定已经执行完毕。
  \answerline{错误。这条语句返回的时候只是cpu已经拉起了一个Kernel，并不一定执行完毕。}
  \item 同一个 stream 里，\texttt{cudaMemcpy}（device 到 host）会等它前面的 kernel 全部完成后才开始拷贝。
  \answerline{正确。同一个stream中不同kernel提交的各种操作都是严格按序执行的。}
  \item kernel 内部的非法访存，会在启动语句处同步地报出来。
  \answerline{错误。kernel内部的报错会等执行它的线程运行到非法操作的时候报出。}
\end{enumerate}
...
\answerline{出bug的原因在于给每个block开了过高的线程数。实验环境的GPU是5090，最大线程数/block为1024，而原程序开了2048，因此被驱动识别为无效参数.
之所以原来不加\texttt{CHECK\_CUDA\_KERNEL()}不会有任何反映，是因为程序中没有检查cuda kernel的错误，只有进行memcpy操作的时候才会暴露出错误。
只来源于评测程序中发现结果不对。}
...
\answerline{这种写法的价值在于可以将一个Kernel需要使用的算力与其问题规模解耦，这样就可以自由地为不同的kernel分配算力。这样做在性能上付出的代价是用的算力定死了，
对不同规模的问题会使用同样的算力，可能导致大规模问题处理很慢。}
...
\answerline{(a) 由每个block被调度执行的顺序决定; (b) 程序的正确性不能依赖block的执行顺序。这条限制是scalable programming model对于程序提出要求。}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \answerline{线性编号为43，它在idx=1的warp内idx=11的lane上。}
  \item 这个 block 一共占多少个 warp？
  \answerline{一共占用了2个warp}
  \item 若 \texttt{blockDim = (33, 1, 1)}，占几个 warp？这样配置浪费在哪里？
  \answerline{一共占用两个warp。这样配置的话idx=1的warp实际上只有1个thread在工作，但是依然占用了一个warp，浪费了另外31个thread的计算资源。}
\end{enumerate}
...
\begin{lstlisting}
测试结果：
warp 内分支 (tid % 2)    :    0.373 ms
按 warp 分支 (tid/32 % 2):    0.191 ms
比值: 1.95
\end{lstlisting}

\answerline{按照warp边界对齐划分分支的kernel运行会快一些，运行时间大约是另一个kernel的一半。
这是因为它的warp内部没有线程束分化，因此每个warp的执行时间都是等于一个thread的运行时间；而按照threadIdx奇偶性
划分分支的kernel每个warp内部都会有属于两个branch的thread，因此每个warp都需要经历两个branch，时间翻倍。实测结果与此略有偏差，可能是因为每个kernel内部的所有thread不是完全同步地执行的，因此时间上略有偏差。

如果两个分支的计算量一大一小，按照thread编号奇偶划分的kernel运行时间由两个分支的总计算量决定，而按照warp边界对齐分的kernel运行时间由计算量较大的一个分支的运行时间决定。}
...
\answerline{(a)因为thread之间的执行顺序是无法保证的，去掉sync代码之后可能某个thread读buf的时候它需要的数据还没有被另一个thread写入。
(b)这个例子中没有某个warp需要的数据传输在warp内部的情况，所有warp需要的数据都是另一个warp写入shared memory的。但是因为固定的硬件对于warp的调度顺序有某种固定倾向，因此某些warp之间的执行顺序相对固定，所以
对应的位置表现出稳定的正确性。}

\prob{3.4}{CONCEPT}
\texttt{\_\_syncthreads} 只能同步本 block 内的 threads，那需要全 grid 同步时，标准做法是什么？

\answerline{标准做法是通过main memory，即GPU的显存来传递数据，并调用\texttt{cudaDeviceSynchronize}，用kernel之间的边界来确保同步性。}
...
shared & 单个Block & 线程块 & 片上 & 用户 \\
global & 整个grid & 应用程序 & 片外 & 用户 \\
constant & 整个grid & 应用程序 & 片外 & 用户 \\
L1 / L2 cache & 单个线程 & 线程 & 片上 & 硬件 \\
\bottomrule
\end{tabular}
\end{center}
...
\answerline{性能差别测不出来是因为constant类型的数组也是存在globa memory里面的，读取速率没有发生变化。constant cache的真正优势在于多个warp都需要访问这个
常数数组的时候，constant会在第一次被调用时加载到shared memory中，后续的warp再访问就会走shared memory带宽，远高于显存带宽。}

\prob{4.4}{CONCEPT}
判断对错，可以顺带补一句理由。
\begin{enumerate}[label=(\alph*)]
  \item local memory 的“local”指作用域私有，它实际上在片外显存里。
  \answerline{正确。CUDA Programming Guide的2.3.3.4章节第一段就是如此描述的。}
  \item 对数组用运行期才知道的下标做索引，可能迫使它被放进 local memory。
  \answerline{正确。CUDA对于无法在编译时确定索引的数组可能会将其放到Local memory中，因为无法直接为它分配thread的regfile资源。}
\end{enumerate}
...
\answerline{提速来自于所有thread进行atomic操作时的访存带宽变大了。进行了修改之后，在每个thread向shared memory中的hist副本进行
累加操作时，所有thread的访存带宽大约是这个kernel享有的所有SM与shared memory的带宽之和，这远远大于每个thread都直接访问global memory中的hist时享有的
显存带宽。}
...
\midrule
GB/s & 1891.9 & 1196.6 & 741.0 & 587.2 & 1652.6 & 867.2 \\
\bottomrule
\end{tabular}
\end{center}

\answerline{当stride <= 8时，kernel体现出的global memory访问带宽持续下降，这是因为每个thread需要4B数据，而global memory的访问粒度是
32B，当stride增大的时候，memory access coalescing的效率逐渐降低，因此带宽逐渐降低。而最后两组实验的带宽突然变大，这个其实是因为给出
的评测样例n刚好为2的幂，\texttt{strided\_copy}实际上只访问了in的一个子集，当stride变大的时候cache hit rate会上升，最后两组实验里有很多的memory access都是走的片上
L2 cache，并不是访问global memory的，带宽因此变大。}
...
实测带宽 (GB/s) & 1567.9 & 1569.7 & 1570.6 & 1574.4 & 1577.7 & 813.2 \\
\bottomrule
\end{tabular}
\end{center}

请回答：(a)~用程序开头打印的“shared memory / SM”和“最大常驻线程 / SM”，手算其中一个的驻留 block 数和 occupancy，和 API 的结果对照。
(b)~带宽为什么随 occupancy 下降？用“延迟隐藏需要足够多的常驻 warp”组织你的解释。
(c)~表中带宽随 occupancy 单调下降，但明显不成正比——从 100\% 到 75\% 带宽掉了多少？从 37.5\% 到 12.5\% 又掉了多少？试解释这个差别。

\answerline{(a) 当shared/block = 13.2 KB时，根据100KB/SM的shared memory总容量计算，一个SM上可以容纳7个block。但是由于SM上最大的驻留thread数量为1536 = 256 * 6，且kernel launch的blockDim参数为256，
因此每个SM最多驻留6个block，occupancy为100\%，与程序计算结果相同；
(b) 当一个SM上有很多warp在执行时，一个warp的访存很容易跟另一个warp的计算进行overlap，从而隐藏计算/访存的延时。而当warp数量不够多时，很可能出现
某个时刻所有的warp都在计算/都在访存，这样这部分延时就暴露出来了。在本例中，当block/SM = 1时，每个SM上只有4个warp，只要它们同时在计算，访存带宽在这个时候就是浪费的，因此导致bandwidth下降
(c) 从 100\% 到 75\% 带宽没掉，从50.0\%到16.7\%大约掉了一半。这与前一问的解释是相关的，带宽掉多少取决于有多少延时没有被盖住，当warp数量足够多的时候，同一个SM的算力和带宽都已经被用满，是warp
在等待SM的处理，这时候降低一些occupancy也不会有带宽的降低；而当warp数量降低到可以让SM的带宽出现空转的临界点时，带宽就会随occupancy显著下降。}
...
\answerline{(a) cudaEvent的计时可以当作kernel耗时写进报告
(b) host计时、不等GPU记录的是CPU拉起kernel的时间，而host计时、等GPU记录的是kernel运行时间加上CPU拉起kernel的时间。}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \answerline{正确。只要CPU中还有别的任务不需要等待这个kernel进行sync，就可以继续执行别的操作，不用等kernel跑完。}
  \item unified memory 下，CPU 访问一页正被 GPU 占用的内存，会触发缺页与页迁移。
  \answerline{正确。内存页被GPU锁住时CPU无法访问。}
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \answerline{错误。tile时不可变的，任何改动操作都会生成一个新的tile；题干的描述是array的性质。}
  \item 对 tile 的一次运算（如两个 tile 相加）由编译器映射到 block 内的多个线程上执行。
  \answerline{正确。}
  \item tile 模型与 SIMT 模型互斥，一个 CUDA 程序只能选一种。
  \answerline{错误。tile和SIMT可以共存于一个CUDA程序中。它们在kenrel层级的编程上是互斥的，一个kernel只能选择一种编程方式。}
\end{enumerate}
...
编号 & \texttt{blockIdx} / \texttt{threadIdx} & \texttt{blockIdx} & \texttt{blockIdx}\\
数据分工 & 线程用全局下标来划分数据 & 用户划分tile，cuTile会自动编译将tile分给不同的block执行 & 用户划分grid，triton自动分配block和thread进行并行\\
边界处理 & \texttt{if} 判断 & tile在编译时会自动按照用户指定的方法处理边界 & 编译时自动按照用户指定的方法处理边界\\
\bottomrule
\end{tabular}
\end{center}
...
\answerline{(a) 由cuTile编译器决定；(b)~线程编号、warp、线程同步}
...
\answerline{主要集中在 kernel 中的计算逻辑。主体代码不用动是因为确定好\texttt{BLOCK\_SIZE}之后，底层的数据搬运和线程划分是由triton编译器自动完成的，用户只负责配置。}
...
\answerline{行号、列号、grid 尺寸还有对应，边界保护被交给TileLang编译器自己处理了。}
...
\midrule
线程到数据的映射 & 用户 & 编译器 & 编译器 & 用户可以选择自己管理，也可以选择交给编译器管理 \\
边界处理 & 用户 & 用户可以定义越界的处理方式，编译器负责越界的判定和处理& 编译器 & 编译器 \\
tile / block 尺寸的选择 & 用户 & 用户 & 用户 & 用户\\
block 内同步 & 用户 & 编译器 & 编译器 & 当用户选择自己控制线程时需要自己管理，否则由编译器管理 \\
\bottomrule
\end{tabular}
\end{center}
...
\answerline{因为TileLang支持对于tile进行存储空间分配和计算的并行方式进行定义，而Triton语言中用户只能定义tile，memory分配和具体的SIMT执行全部由编译器管理。}
...
\begin{enumerate}[label=(\alph*)]
  ...
  \answerline{错误。PTX是一种汇编程序，需要在GPU执行时再编译为对应GPU的binary code}
  \item 只嵌入了 \texttt{sm\_70} SASS 的可执行文件，能在 compute capability 9.0 的卡上运行。
  \answerline{错误。\texttt{sm\_70}与compute capability 9.0的卡不兼容，同时文件中缺乏ptx，因此无法编译出可兼容的binary code}
...
\answerline{报错信息为 CUDA error cudaErrorNoKernelImageForDevice at \texttt{m0\_env/01\_hello.cu}:11: no kernel image is available for execution on the device}
...
\answerline{程序可以正常运行。PTX是在程序加载的时候由设备驱动器进行JIT编译变成机器码的。}

\prob[Optional]{8.3}{CONCEPT}
请简单说明 Runtime API 与 Driver API 各自的定位。\texttt{cudaMalloc} 属于哪个？

\answerline{Runtime API是留给用户方便编程的接口，而Driver API是暴露给NVIDIA DRIVER的API接口。\texttt{cudaMalloc} 属于前者。}
