...
\begin{enumerate}[label=(\alph*)]
  ...
  答：错误。GPU的算力是所有SIMT处理核算力的加总，单核性能不一定比得上CPU的单核。
  \item HBM 的“高带宽”指大块连续访问时的吞吐，零散的随机访问达不到标称值。
  答：正确。零散的访问地址可能导致映射到HBM中的物理地址非常集中，无法利用HBM内部的并行度，用不满带宽。
  \item 严格串行的迭代算法（每步依赖上一步的结果），即使换一块算力更强的 GPU 也快不了多少。
  答：正确。严格的串行算法无法通过SIMT的方式来进行访存和计算的overlap，因此本质上是用单个core进行串行计算，而GPU的算力提升一般来源于核心数量的提升，单核性能提升不显著。
  \item “算力 1000 TFLOPS”意味着每次运算的延迟是 $10^{-15}$ 秒。
  答：错误。算力是一个吞吐上的概念，1000TFLOPS也可能是用 $10^{-13}$ 秒完成100OPS的计算。
\end{enumerate}

\prob{1.2}{CONCEPT}
Session 1 讲座里提过“N 方过百万”这个例子。总计算量 $10^{12}$ FLOP 在当代 GPU 上的运算时间大概是毫秒级，那为什么一个严格在线的串行算法仍然做不到几秒内跑完？（从“延迟”和“吞吐”的角度考虑）
答：严格串行的算法由于数据依赖无法并行执行，此时吞吐不能靠提高计算的并行度来提高，而是严格与延迟成反比，因此在GPU这种单核能力弱、靠提高并发度来
...
warp & GPU调度thread的最小单元 & SM中的一个分区 & L0, L1 (不含shared memory) & warp之间的通讯通过L0, L1进行，通过CUDA提供的warp_primitive调用 \\
block / CTA & thread协作的基本单位 & SM & L0, L1 (含shared memory) & 使用__syncthreads同步机制，通信通过shared memory进行 \\
...
答：SIMD与SIMT的区别在于thread和pc的数量，SIMD中是一个thread, 一个pc同时对多个data进行同一种操作，而SIMT则是同时调度多个thread的执行，每个thread有自己的pc并且可以单独执行。
...
答：(a) GPU的单核能力弱于CPU，因为CPU中有分支预测以及prefetch等手段来给程序喂数据，降低流水线stall的概率，而GPU中没有这种技术，导致单核计算性能明显更弱；
...
答：(a) 为了保证CPU读数据的时候所有thread都已经执行完，CPU能得到正确的结果。原先的版本中这个同步发生在CUDA_CHECK_KERNEL()这个函数里调用cudaDeviceSynchronize()的这个函数。
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：错误。这条语句返回的时候只是cpu已经拉起了一个Kernel，并不一定执行完毕。
  \item 同一个 stream 里，\texttt{cudaMemcpy}（device 到 host）会等它前面的 kernel 全部完成后才开始拷贝。
  答：正确。同一个stream中不同kernel提交的各种操作都是严格按序执行的。
  \item kernel 内部的非法访存，会在启动语句处同步地报出来。
  答：错误。kernel内部的报错会等执行它的线程运行到非法操作的时候报出。
\end{enumerate}
...
答：出bug的原因在于给每个block开了过高的线程数。实验环境的GPU是5090，最大线程数/SM为1536，而原程序开了2048，这意味着一个block不能被调度到一个SM上执行.
之所以原来不加CHECK_CUDA_KERNEL()不会有任何反映，是因为程序中没有检查cuda kernel的错误，只有进行memcpy操作的时候才会暴露出错误。
只来源于评测程序中发现结果不对。
...
答：这种写法的价值在于可以将一个Kernel需要使用的算力与其问题规模解耦，这样就可以自由地为不同的kernel分配算力。这样做在性能上付出的代价是用的算力定死了，
...
答：(a) 由每个block被调度执行的顺序决定; (b) 程序的正确性不能依赖block的执行顺序。这条限制是scalable programming model对于程序提出要求。
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：线性编号为43，它在idx=1的warp内idx=11的lane上。
  \item 这个 block 一共占多少个 warp？
  答：一共占用了2个warp
  \item 若 \texttt{blockDim = (33, 1, 1)}，占几个 warp？这样配置浪费在哪里？
  答：一共占用两个warp。这样配置的话idx=1的warp实际上只有1个thread在工作，但是依然占用了一个warp，浪费了另外31个thread的计算资源。
\end{enumerate}
...
答: 按照warp边界对齐划分分支的kernel运行会快一些，运行时间大约是另一个kernel的一半。
...
答：(a)因为thread之间的执行顺序是无法保证的，去掉sync代码之后可能某个thread读buf的时候它需要的数据还没有被另一个thread写入。
...
答：标准做法是通过main memory，即GPU的显存来传递数据，并调用\texttt{cudaDeviceSynchronize}，用kernel之间的边界来确保同步性。
...
答：性能差别测不出来是因为constant类型的数组也是存在globa memory里面的，读取速率没有发生变化。constant cache的真正优势在于多个warp都需要访问这个
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：正确。CUDA Programming Guide的2.3.3.4章节第一段就是如此描述的。
  \item 对数组用运行期才知道的下标做索引，可能迫使它被放进 local memory。
  答：正确。CUDA对于无法在编译时确定索引的数组可能会将其放到Local memory中，因为无法直接为它分配thread的regfile资源。
\end{enumerate}
...
答：提速来自于所有thread进行atomic操作时的访存带宽变大了。进行了修改之后，在每个thread向shared memory中的hist副本进行
...
答：当stride <= 8时，kernel体现出的global memory访问带宽持续下降，这是因为每个thread需要4B数据，而global memory的访问粒度是
32B，当stride增大的时候，memory access coalescing的效率逐渐降低，因此带宽逐渐降低。而最后两组实验的带宽突然变大，这个其实是因为给出
的评测样例n刚好为2的幂，strided_copy实际上只访问了in的一个子集，当stride变大的时候cache hit rate会上升，最后两组实验里有很多的memory access都是走的片上
...
shared/block   理论 block/SM  occupancy   实测带宽
     0.0 KB          6          100.0%     1567.9 GB/s
    13.2 KB          6          100.0%     1569.7 GB/s
    15.0 KB          6          100.0%     1570.6 GB/s
    18.0 KB          5           83.3%     1574.4 GB/s
    29.0 KB          3           50.0%     1577.7 GB/s
    55.0 KB          1           16.7%      813.2 GB/s
...
答：(a) 当shared/block = 13.2 KB时，根据100KB/SM的shared memory总容量计算，一个SM上可以容纳7个block。但是由于SM上最大的驻留thread数量为1536 = 256 * 6，且kernel launch的blockDim参数为256，
...
答：(a) cudaEvent的计时可以当作kernel耗时写进报告
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：正确。只要CPU中还有别的任务不需要等待这个kernel进行sync，就可以继续执行别的操作，不用等kernel跑完。
  \item unified memory 下，CPU 访问一页正被 GPU 占用的内存，会触发缺页与页迁移。
  答：正确。内存页被GPU锁住时CPU无法访问。
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：错误。tile时不可变的，任何改动操作都会生成一个新的tile；题干的描述是array的性质。
  \item 对 tile 的一次运算（如两个 tile 相加）由编译器映射到 block 内的多个线程上执行。
  答；正确。
  \item tile 模型与 SIMT 模型互斥，一个 CUDA 程序只能选一种。
  答：错误。tile和SIMT可以共存于一个CUDA程序中。它们在kenrel层级的编程上是互斥的，一个kernel只能选择一种编程方式。
\end{enumerate}
...
数据分工 & 线程用全局下标来划分数据 & 用户划分tile，cuTile会自动编译将tile分给不同的block执行 & \\
边界处理 & \texttt{if} 判断 & tile在编译时会自动按照用户指定的方法处理边界 & \\
\bottomrule
\end{tabular}
\end{center}
...
答：主要集中在 kernel 中的计算逻辑。主体代码不用动是因为确定好BLOCK_SIZE之后，底层的数据搬运和线程划分是由triton编译器自动完成的，用户只负责配置。
...
答：行号、列号、grid 尺寸还有对应，边界保护被交给TileLang编译器自己处理了。
...
答：因为TileLang支持对于tile进行存储空间分配和计算的并行方式进行定义，而Triton语言中用户只能定义tile，memory分配和具体的SIMT执行全部由编译器管理。
...
\begin{enumerate}[label=(\alph*)]
  ...
  答：错误。PTX是一种汇编程序，需要在GPU执行时再编译为对应GPU的binary code
  \item 只嵌入了 \texttt{sm\_70} SASS 的可执行文件，能在 compute capability 9.0 的卡上运行。
  答：错误。sm_70与compute capability 9.0的卡不兼容，同时文件中缺乏ptx，因此无法编译出可兼容的binary code
...
答：报错信息为 CUDA error cudaErrorNoKernelImageForDevice at m0_env/01_hello.cu:11: no kernel image is available for execution on the device
...
答：程序可以正常运行。PTX是在程序加载的时候由设备驱动器进行JIT编译变成机器码的。

\prob[Optional]{8.3}{CONCEPT}
请简单说明 Runtime API 与 Driver API 各自的定位。\texttt{cudaMalloc} 属于哪个？

答：Runtime API是留给用户方便编程的接口，而Driver API是暴露给NVIDIA DRIVER的API接口。\texttt{cudaMalloc} 属于前者。
